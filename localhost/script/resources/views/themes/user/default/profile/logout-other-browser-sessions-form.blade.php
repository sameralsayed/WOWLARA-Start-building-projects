<x-jet-action-section>
    <x-slot name="title">
        {{ __('AA.Browser Sessions') }}
    </x-slot>

    <x-slot name="description">
        {{ __('AA.Manage and logout') }}
    </x-slot>
    ...
    <x-slot name="content">
                            ...
                            <div>
                                <div class="text-xs text-gray-500">
                                    ...
                                    @if ($session->is_current_device)
                                        <span class="text-green-500 font-semibold">{{ __('AA.This device') }}</span>
                                    @else
                                        {{ __('AA.Last active') }} {{ $session->last_active }}
                                    @endif
                                </div>
                            </div>
                        </div>
        ...
        <div class="flex items-center mt-5">
            <x-jet-button wire:click="confirmLogout" wire:loading.attr="disabled">
                {{ __('AA.Logout Other Browser Sessions') }}
            </x-jet-button>
