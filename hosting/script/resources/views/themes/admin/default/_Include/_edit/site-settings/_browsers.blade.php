@if(str_contains(url()->current(), '/admin/site-info'))
    <h6>{!!$admin["notio"]!!} {{__('AA.To allow browsers')}} {{$get->name}} {{__('AA.Language')}}</h6><hr>
    @else
    <h6>{!!$admin["notio"]!!} {{__('AA.The browsers that you want to allow him to access the site')}}</h6><hr>
@endif
{!!$admin["form"]!!}<ul>
